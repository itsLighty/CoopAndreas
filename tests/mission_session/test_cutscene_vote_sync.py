...
class CutsceneVoteSyncTests(unittest.TestCase):
    ...
    def test_scm_remains_the_only_visual_start_path(self):
        ...
        self.assertIn("SkipCurrentCutsceneImmediately()", send)
    ...
    def test_client_vote_never_performs_local_skip(self):
        ...
        self.assertIn("SkipCurrentCutsceneImmediately();", apply_skip)
        immediate_skip = function_body(self.client, "void CCutsceneVoteManager::SkipCurrentCutsceneImmediately(")
        self.assertIn("CCutsceneMgr::ms_wasCutsceneSkipped = true", immediate_skip)
        self.assertIn("if (CCutsceneMgr::ms_running)", immediate_skip)
        self.assertIn("CCutsceneMgr::FinishCutscene();", immediate_skip)
        process = function_body(self.client, "void CCutsceneVoteManager::Process(")
        self.assertIn("if (CCutsceneMgr::ms_running)", process)
        self.assertIn("SkipCurrentCutsceneImmediately();", process)
        receive = function_body(self.opcodes, "void COpCodeSync::HandlePacket(")
        self.assertIn("header.opcode == 0x02E7", receive)
        self.assertIn("CCutsceneVoteManager::SkipCurrentCutsceneImmediately();", receive)

    ...
    def test_all_windows_resources_match_protocol_version(self):
        for relative in (
            ...
            with self.subTest(relative=relative):
                ...
                self.assertIn("FILEVERSION 0,3,8,0", contents)
                self.assertIn("PRODUCTVERSION 0,3,8,0", contents)
                self.assertIn('VALUE "FileVersion", "0.3.8-alpha"', contents)
                self.assertIn('VALUE "ProductVersion", "0.3.8-alpha"', contents)
